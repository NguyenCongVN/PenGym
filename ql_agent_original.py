...
class TabularQLearningAgent:
    ...
    def train(self):
        ...
        while self.steps_done < self.training_steps:
            ...
            if num_episodes % 10 == 0 and self.verbose:
                ...
                logger.fatal(f"\tsteps done = {self.steps_done} / "
                      f"{self.training_steps}")
                logger.fatal(f"\treturn = {ep_return}")
                logger.fatal(f"\tgoal = {goal}")

        self.logger.close()
        if self.verbose:
            logger.fatal("Training complete")
            logger.fatal(f"\nEpisode {num_episodes}:")
            logger.fatal(f"\tsteps done = {self.steps_done} / {self.training_steps}")
            logger.fatal(f"\treturn = {ep_return}")
            logger.fatal(f"\tgoal = {goal}")
    ...
    def run_eval_episode(self,
        ...
        if render:
            logger.fatal("\n" + line_break)
            logger.fatal(f"Running EVALUATION using epsilon = {eval_epsilon:.4f}")
            logger.fatal(line_break)
            env.render()
        ...
        while not done and not env_step_limit_reached:
            ...
            if render:
                logger.fatal("\n" + line_break)
                logger.fatal(f"Step {steps}")
                logger.fatal(line_break)
                logger.fatal(f"Action Performed = {env.action_space.get_action(a)}")
                env.render()
                logger.fatal(f"Reward = {r}")
                logger.fatal(f"Done = {done}")
                logger.fatal(f"Step limit reached = {env_step_limit_reached}")
                input("Press enter to continue..")

                if done or env_step_limit_reached:
                    logger.fatal("\n" + line_break)
                    logger.fatal("EPISODE FINISHED")
                    logger.fatal(line_break)
                    logger.fatal(f"Goal reached = {env.goal_reached()}")
                    logger.fatal(f"Total steps = {steps}")
                    logger.fatal(f"Total reward = {episode_return}")
